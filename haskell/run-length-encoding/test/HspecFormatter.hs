  ...
  where
    ...
    handleItemDone (_, requirement) item = do
        ...
        case itemResult item of
          Success -> do
            BS.putStrLn $ encode $ TestResult { name = requirement, status = Pass, message = Nothing, taskId }
            addTestResult TestResult { name = requirement, status = Pass, message = Nothing, taskId }
          ...
          Failure _ failureReason ->
            ...
            in print (encode result) *> addTestResult result
      where
...
formatter _config = do
  getTaskId <- Manifest.getManifest >>= \case
    Nothing -> do
      IO.hPutStrLn IO.stderr "Could not find manifest."
      pure $ \_ -> Nothing
    Just m -> pure $ Manifest.getTaskId m
